...
La méthode de calcul de l'erreur standard donnée par Hanley et McNeil (package MKmisc[@mkmisc]) sera
...
Une limite acceptable sera une \gls{auc} > \num{0.8}. L’erreur standard sera calculée par la méthode de Hanley et McNeil (package `MKmisc` [@mkmisc]) avec une erreur admissible à \num{0.075}. Des intervalles de confiance par zone (méthode de Clopper and Pearson) seront tracés autour des probables points d'inflexion.
