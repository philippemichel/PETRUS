...
Les différences éventuelles dans les mesures seront recherchées tout d'abord par un test de \textsc{Kappa-Cohen} (package `irr` [@irr]) en catégorisant les résultats en normal/pathologique avec un seuil admissible à \num{0.4} (accord modéré)[^1].

Puis un analyse en sensibilité /spécificité sera réalisée en prenant la mesure des expert comme gold-standard.

[^1]: Le seuil a été choisi bas en raison de la faible taille des échantillons (10 cas)

Si les résultats d'un centre semblent insuffisants l'information sera transmise à l'investigateur coordonateur.
...
Le diagramme de \textsc{Bland} & \textsc{Altmann} est classiquement utilisé pour présenter des mesures réalisées par deux cliniciens mais ne s'applique pas pour plus de cliniciens. Les diagrammes de \textsc{Bland} & \textsc{Altmann} seront tracés pour chaque paire de cliniciens (trois diagrammes) Les résultats seront présentés par un diagramme présentant en abscisse la mesure & en ordonnée la moyenne, les examinateurs étant différentiés par la couleur du point (graphique @fig-ba1). Les différences seront évaluées par la méthode proposée par \textsc{S. Möller}[@moller11] sur un diagramme avec en abscisse la moyenne des mesures pour chaque sujet & en ordonnée l'écart-type entre les trois mesures -- (graphique @fig-ba2).
