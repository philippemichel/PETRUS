...
Le risque $\alpha$ retenu sera de \num{0.05} & la puissance de \num{0.8}. Vu la taille de l'échantillon l'hypothèse de normalité peut être retenue.



## Taille de l'échantillon

Parmi les cas suspects d’\gls{acg} inclus dans l’étude, les cas d'\gls{acg} positif et \gls{acg} négatif seront séparés et la mesure du marqueur diagnostique « \gls{dgno}) » sera réalisée pour chaque sujet et ce afin de calculer un intervalle de confiance unilatéral de \qty{97.5}{\percent} pour la limite inférieure de l'aire sous la courbe ROC (\gls{auc}) du marqueur.

La valeur de l'\gls{auc} de l'échantillon est supposée être de \num{0.8} (bon marqueur). La méthode de calcul de l'erreur standard donnée par \textsc{Hanley} et \textsc{McNeil} (package `MKmisc`[@mkmisc]) sera utilisée pour calculer la limite de l'intervalle de confiance.
...
Toutes les mesures des investigateurs seront contrôlées par deux experts. Ceux ci sont désignés par randomisation parmi une liste de trois experts (table de randomisation). On validera la mesure de l'investigateur par la comparaison entre sa mesure & la moyenne des mesures des deux experts & le classement en normal/pathologique. La mesure de l'investigateur sera considérée comme valide si elle diffère de moins de \qty{1}{\mm} de la moyenne des mesures des experts. En cas de divergence c'est la mesure des experts qui sera retenue. 
